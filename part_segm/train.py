...
parser.add_argument('--model', default='acnn_segm_rings_v2', help='Model name [default: model]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            pointclouds_pl, labels_pl, normals_pl, cls_labels_pl = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT)
            ...
            # Get model and loss
            pred, end_points = MODEL.get_model(pointclouds_pl, cls_labels_pl, normals_pl, is_training_pl, bn_decay=bn_decay)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            _, acc_cat, acc_inst = eval_one_epoch(sess, ops, test_writer)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label, batch_cls_label = get_batch(TEST_DATASET, test_idxs, start_idx, end_idx)
